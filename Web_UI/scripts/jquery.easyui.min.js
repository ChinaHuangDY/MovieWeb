...
(function ($) {
    ...
    function _362(_366, _367, _368) {
        ...
        for (var i = 0; i < tabs.length; i++) {
            var tab = tabs[i];
            //lijj  replace(/<[^>]+>/g, "") 去掉tab选择卡标题中的html 代码判断
            if (tab.panel("options").title.replace(/<[^>]+>/g, "") == _367.replace(/<[^>]+>/g, "")) {
                if (_368) {
                    tabs.splice(i, 1);
                }
                return tab;
...
(function ($) {
    ...
    $.fn.numberbox.defaults = $.extend({}, $.fn.textbox.defaults, {
        inputEvents: {
            ...
            ////var baseNum = 1;
            ////for (var i = 0; i < opts.precision; i++) {
            ////    baseNum = baseNum * 10;
            ////}
            ////var val = Math.floor(s * baseNum) / baseNum;
            //var temp = s.split('.');
            //var ANum = temp[0];
            //var BNum = '';
            //if (temp.length > 1) {
            //    BNum = temp[1].substring(0,opts.precision);
            //}
            //var val = ANum;
            //if (BNum!='')
            //{
            ...
            //    val=parseFloat(opts.min.toFixed(opts.precision));
            //}else{
            //if(typeof (opts.max)=="number"&&val>opts.max){
            //    val=parseFloat(opts.max.toFixed(opts.precision));
            //}
            //}
            //}
            //return val;
...
(function ($) {
    ...
    function _5fb(_5fc) {
        ...
        _600.add(_601).children("div.datagrid-body").css({ marginTop: _607, height: (isNaN(parseInt(opts.height)) ? "" : (_5ff - _608) - 1), minHeight: (_60a ? _60a - _609 - _608 : ""), maxHeight: (_60b ? _60b - _609 - _608 : "") });
    ...
    function _60c(_60d, _60e, _60f) {
        ...
        if (opts.height == "auto") {
            ...
            var _611 = dc.body1.parent().parent();
            var _612 = dc.body2;
            var _613 = _614(_612);
            var _615 = _613.height;
            if (_613.width > _612.width()) {
                //lijj 2017-07-11 去掉附件列表下方空白行
                if (opts.showBottomScroll) {
                    _615 += 18;
                } else {
                    try {
                        //去掉下方重复边框线
                        dc.body1.parent().parent().parent().parent().css("borderBottom", "none");
                    } catch (e) { }
                }
            }
            //if (!opts.fit)
            ...
            //        //去掉下方重复边框线
            //        dc.body1.parent().parent().parent().parent().css("borderBottom", "none");
            //        if (opts.data.length == 0) { //如果列表没有数据，头高和列表高度保存一致
            //            _615=_615-5;
            //        }
            //    } catch (e) { }
            //}
    ...
    function _629(_62a) {
        ...
        _62c.panel($.extend({}, opts, {
            id: null, doSize: false, onResize: function (_62d, _62e) {
                ...
                try {
                    $('.datagrid-body').getNiceScroll().show();
                    $('.datagrid-body').getNiceScroll().resize();
                    $(".nicescroll-rails.nicescroll-rails-vr").css("width", "7px");
                } catch (e) {
    ...
    function _671(_6b7, data) {
        ...
        try {
            ...
            $('.datagrid-body').niceScroll({
                ...
                horizrailenabled: opts.isShowHor||false,
    ...
    function _6e2(_6e3, _6e4) {
        ...
        opts.finder.getTr(_6e3, "", "allbody").addClass("datagrid-row-selected").css("background-color", "#aed9fb");
        if (opts.idField) {
    ...
    function _6dd(_6e9, _6ea) {
        ...
        opts.finder.getTr(_6e9, "", "selected").removeClass("datagrid-row-selected").css("background-color", "");
        if (opts.idField) {
    ...
    var _7b7 = {
        render: function (_7b8, _7b9, _7ba) {
            ...
            opts.striped = true;  //lijj 2017-09-25 强制显示斑马线 
            if (_7c2) {
...
(function ($) {
    ...
    function _9ae(_9af) {
        ...
        $.fn.datebox.defaults.panelWidth
        if (_9b3.closed) {
            if ($.fn.datebox.defaults.panelWidth == opts.panelWidth || !opts.panelWidth) {
                opts.panelWidth = _9b1._outerWidth();
            }
            _9b2.panel("panel").show().css({ zIndex: ($.fn.menu ? $.fn.menu.defaults.zIndex++ : ($.fn.window ? $.fn.window.defaults.zIndex++ : 99)), left: -999999 });
            _9b2.panel("resize", { width: (opts.panelWidth), height: opts.panelHeight });
