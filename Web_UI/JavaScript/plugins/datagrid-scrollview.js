    ...
    onBeforeRender: function (target) {
        ...
        function scrolling() {
            if (dc.body2.is(':empty')) {
                ...
                try
                {
                    top=topDiv.position().top + topDiv._outerHeight() - headerHeight;
                    bottom=bottomDiv.position().top - headerHeight;
                } catch (e) {
        ...
        function createHeaderExpander() {
            ...
            if (!hasExpander) {
                if (opts.frozenColumns && opts.frozenColumns.length) {
                    opts.frozenColumns[0].splice(0, 0, { field: '_expander', expander: true, width: 50, resizable: false, fixed: true });
                } else {
                    opts.frozenColumns = [[{ field: '_expander', expander: true, width: 50, resizable: false, fixed: true }]];
                }

                var t = dc.view1.children('div.datagrid-header').find('table');
                var td = $('<td rowspan="' + opts.frozenColumns.length + '"><div class="datagrid-header-expander" style="width:49px;"></div></td>');
                if ($('tr', t).length == 0) {
